...
class PivotEqualsTest extends TestCase
{
    ...
    public function returns_correct_statement()
    {
        ...
        $sql = "SELECT `book_id` FROM `author_book` WHERE `author_id` = ?";

        $this->assertEquals($this->trimWhiteSpace($sql),$this->trimWhiteSpace($query->query()),"PivotEquals::query() return invaild statement");
