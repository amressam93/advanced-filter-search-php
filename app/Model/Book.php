...
class Book extends Model
{
    ...
    public function sql()
    {
       ...
       (SELECT GROUP_CONCAT(`name`) FROM `authors`
       WHERE `id` IN (SELECT `author_id` FROM `author_book` WHERE `book_id` = `{$this->table}`.`id`)) AS `authors` ,
       (SELECT GROUP_CONCAT(`name`) FROM `languages`
       WHERE `id` IN (SELECT `language_id` FROM `book_language` WHERE `book_id` = `{$this->table}`.`id`)) AS `languages` ,
       (SELECT GROUP_CONCAT(`name`) FROM `locations`
       WHERE `id` IN (SELECT `location_id` FROM `book_location` WHERE `book_id` = `{$this->table}`.`id`)) AS `locations` ,
       (SELECT GROUP_CONCAT(`name`) FROM `covers`
       WHERE `id` IN (SELECT `cover_id` FROM `book_cover` WHERE `book_id` = `{$this->table}`.`id`)) AS `covers`
       FROM `{$this->table}` LEFT JOIN `categories` ON `categories`.`id` = `{$this->table}`.`category_id`";
    }
    ...
    {
        return $this->sortImplode(explode(",",$string),$joint);
    }
